...
func testCopier(t *testing.T, store types.Storager, f types.StorageFeatures) {
	if f.Copy && f.Write && f.Read && f.Delete {
		// copy a file to a non-existent file
		...
		if f.CreateDir {
			...
			if !f.VirtualDir {
				...
				// virtual dir
				Convey("When Copy to an existing virtual dir", func() {
					srcSize := rand.Int63n(4 * 1024 * 1024) // Max file size is 4MB
					r := io.LimitReader(randbytes.NewRand(), srcSize)
					src := uuid.New().String()
