...
func generateInfo(data *Data, path string) {
	...
	for serviceName, infos := range data.InfosMap {
		...
		// Generate field bits
		f.CommentF("Field index in %s bit", serviceNameC)
		consts := f.Const()
		for k, v := range infos {
			consts.Field(
		...
		st.Line()
		st.Comment("bit used as a bitmap for object value, 0 means not set, 1 means set")
		st.Field("bit", "uint64")
		...
		for _, v := range infos {
			...
			f.Function("Get"+v.DisplayName()).
				...
				Result("", "bool").
				Body(
					gg.If(gg.StringF("m.bit & %sIndex%s != 0",
						serviceNameC, templateutils.ToPascal(v.Name))).
						Body(
